...
3. **CBAM Registry**가 인증서 수 = SEE × (1 + mark-up) − Free benchmark × CBAM factor 계산 (× EUA)
4. **K-ETS 차감** (Art.9): 한국에서 이미 지불한 carbon price를 차감
5. **Net CBAM cost** 확정 → 인증서 구매·제출

**수식 한 줄** (IR 2025/2620):
```
인증서 수 = SEE × (1 + mark-up) − Free benchmark × CBAM factor   (음수면 0)
Gross     = 인증서 수 × EUA
Net       = Gross − K-ETS credit (verified)
연간 부담 = Net × EU 수출량
CBAM factor = 1 − Phase-in   (2026 97.5% → 2030 51.5% → 2034 0%)
```

> ⚠️ benchmark 초과분은 **2026년부터 전액** 부과됩니다. phase-in으로 늘어나는 건 benchmark분에 대한 무상할당 공제가 줄어드는 몫입니다.
>
> 🧾 **인증서 가격**: 2026년분은 분기별 EUA 경매 평균가(집행위 발표 Q1 €75.36, Q2 €75.28 — 미발표 분기는 사이드바 EUA)이고, 2027년분부터는 주간 평균입니다.

### 1.3 Phase-in 일정 (무상할당 공제 축소)

| 연도 | Phase-in | CBAM factor (무상할당 공제) |
|---|---|---|
...
> 💡 **2030년이 변곡점**. 무상할당 공제가 77.5% → 51.5%로 크게 줄어 부담이 가파르게 늘어납니다. 단, benchmark 초과분은 2026년부터 이미 전액 부과됩니다.

#### 규제 시나리오: 2040 개편안 (제안·미확정)

사이드바 **규제 시나리오**에서 2026-07-17 집행위 ETS 개편안(COM(2026) 616)을 고를 수 있습니다. 기본값은 현행법입니다.

| 연도 | 현행법 무상할당 공제 | 개편안 무상할당 공제 |
|---|---|---|
| 2028 | 90.0% | 91.5% |
| 2029 | 77.5% | 81.0% |
| 2030 | 51.5% | 59.0% |
...
> ⚠️ 의회·이사회 입장이 아직 없는 **제안**입니다. 수치가 바뀔 수 있으니 비교용으로만 쓰세요.
...
| **분석 연도 (2026~2038)** | **2026 (현재 충격)** vs **2030 (변곡점)** vs **2034 (완전 시행)** 3개 비교 권장. |
| **규제 시나리오** | 기본은 현행법. '2040 개편안(제안)'을 고르면 2028~2037년 무상할당 공제가 더 남는 경우와 비교할 수 있음. |
...
- 기본값 30%: 무상할당분을 뺀 '실제 지불분'만 차감된다고 가정 (근거는 FAQ Q3)
- 낙관적 시나리오: 50~100% (무상할당 축소 또는 한·EU 협의로 인정 범위 확대 가정)
- 비관적 시나리오: 차감 0% (차감 거부 가정)

K-ETS 가격 ₩30,000/tCO₂(2026-09 KAU25) × SEE 2.127 ≈ ₩64,000/t product ≈ €41/t가 명목 최대치이고, 기본 30%를 적용하면 약 €12.3/t를 차감합니다. POSCO처럼 SEE 큰 기업일수록 효과 큼.
...
8. **탭 ⑩**: EU CBAM 최신 공지 (수동 큐레이션 + 월 1회 자동 수집)
...
### Q1. POSCO의 2026년 부담이 벌써 톤당 €70 가까이 나오는 이유는?

CBAM은 benchmark 초과분에 phase-in을 곱하는 방식이 아닙니다. **내재배출에서 무상할당 공제(benchmark × CBAM factor)를 뺀 만큼** 인증서를 사야 합니다 (IR 2025/2620, 집행위 CBAM Q&A 3.7–3.8). 2026년 CBAM factor는 97.5%라서:

- POSCO: (2.127 − 1.370 × 0.975) × EUA €87 ≈ **€69/t**
- benchmark 초과분(0.757)은 첫해부터 전액, benchmark분은 2.5%만 부과
- 2034년에는 공제가 0이 되어 내재배출 전량(2.127 × EUA)에 부과

TTI Korea(2026)의 POSCO €73.1/t도 같은 방식입니다 (확정 전 benchmark 1.30 사용).

※ 2026-09-25 이전 버전은 `(SEE − benchmark) × phase-in × EUA`로 계산해 초기 연도 부담을 수십 배 낮게 보여줬습니다. 규정에 맞게 고쳤습니다.
...
### Q3. K-ETS 차감 기본값이 30%인 근거는?

CBAM 규정 Art.9는 수출국에서 **실제로 지불한** 탄소가격만 차감을 인정하고, 무상할당 같은 보전분은 빼고 계산합니다. TTI Korea(2026)는 K-ETS 4기 무상할당(69.2%)을 반영한 차감액을 €2.92/tCO₂로 추정했습니다 — K-ETS 가격(~€9)의 약 30%, EU 가격의 3.4%이며 POSCO 기준 톤당 €2.51입니다 ([CBAM Country Intelligence: Korea 2026](https://terawatttimes.org/cbam-country-intelligence-korea-2026/)). 기본값 30%는 이 비율을 반영한 것입니다.

※ 2026-09 이전 기본값은 50%였으나 과대 추정이라 30%로 낮췄습니다. 무상할당이 줄거나 한·EU 협의로 인정 범위가 넓어지면 사이드바에서 올려 보세요.
...
- **2026**: benchmark 초과분은 이미 전액 부과 — 고배출 공정은 첫해부터 부담이 큼
- **2030**: 무상할당 공제가 51.5%로 줄어 부담 급증 → **CCS 가동개시 목표 연도**
- **2034**: 공제 0 → 내재배출 전량 부과 (benchmark 이하 공정도 부담 발생)

CCS 프로젝트의 **2030 가동 목표**가 합리적인 이유 — 공제가 빠르게 줄어드는 구간 전에 배출을 낮춰야 회피액이 커집니다.
...
5. **K-ETS 차감 비율** — 기본 30%(무상할당분 제외 가정). 실제 인정 범위는 EU 이행규정·검증에 따라 0~100% 변동
6. **CCS ramping curve** — 사례별 학습 곡선 다름 (default 70/85/100%)
7. **CAPEX 일시 집중** — 단순화 (실제는 EPC 기간 분할)
8. **물가상승률 미반영** — 모든 가격은 실질(real) 기준
9. **CSCF(교차부문 보정계수)** — 2026~2030은 적용되지 않아 1.0 (집행위 COM(2026) 619). 2031년 이후는 미반영
