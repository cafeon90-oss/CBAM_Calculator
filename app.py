...
핵심 수식 (IR 2025/2620):
  CBAM cost = (SEE − Free EU benchmark × CBAM factor) × EUA × import volume
  CBAM factor = 1 − Phase-in  (2026: 97.5% → 2034: 0%)

지표 정의:
  SEE     [tCO₂/t]     Specific Embedded Emissions
  Phase-in factor       2026: 2.5% → 2034: 100% — benchmark분 무상할당 공제의 축소율
...
CCUS_SECTOR_FIT = {
    ...
    "steel_BF_BOF":   ["MHI_KS21", "Cansolv_DC103", "Aker_S26",
                        "CAP_B12C", "CaL", "MEA_baseline"],
    # 철강 EAF (CO₂ 5~10%, 저농도)
    "steel_EAF":      ["TSA_Solid", "Biphasic_DMX", "MEA_baseline"],
    # 시멘트 (calcination CO₂, CaL 자연 통합)
    "cement":         ["CaL", "Aker_S26", "MHI_KS21",
                        "CAP_B12C", "MEA_baseline"],
    # 알루미늄 (anode CO₂ + 전력 grid)
    "aluminum":       ["MEA_baseline", "Cansolv_DC103", "Biphasic_DMX"],
    # 비료 NH₃ (고농도 부생 CO₂)
    "fertilizer_NH3": ["MEA_baseline", "K2CO3_KIERSOL", "Cansolv_DC103"],
    # 수소 SMR (PSA off-gas 고농도)
    "hydrogen_SMR":   ["MEA_baseline", "Cansolv_DC103", "Biphasic_DMX",
...
USAGE_MD_LOCAL = Path(__file__).parent / "USAGE.md"
USAGE_SVG_LOCAL = Path(__file__).parent / "cbam_mechanism.svg"


@st.cache_data(ttl=3600)
def load_usage_md():
    """USAGE.md 파일 로드. 실패 시 fallback 안내."""
    try:
        if USAGE_MD_LOCAL.exists():
            return USAGE_MD_LOCAL.read_text(encoding="utf-8"), "auto"
    except Exception:
        pass
    return "(USAGE.md를 불러올 수 없습니다.)", "fallback"


@st.cache_data(ttl=3600)
def load_usage_svg():
    """cbam_mechanism.svg 파일 로드."""
    try:
        if USAGE_SVG_LOCAL.exists():
            return USAGE_SVG_LOCAL.read_text(encoding="utf-8"), "auto"
    except Exception:
        pass
...
# ======================================================================
# Phase-in factor — benchmark분 무상할당 공제가 줄어드는 비율 (= 1 − CBAM factor), 2026~2034
PHASE_IN_FACTORS = {
...
PRESETS = {
    ...
    "kr_sk_h2_blue": {
        ...
        "description": "SMR + CCS 90% — Gray H₂ 대비 CBAM 부담 대부분 회피",
...
REFS = {
    ...
    "EU_IR_2025_2620": {
        "cat": "regulation",
        "date": "2025-12-16 (adopted) · applies from 2026-01-01",
        "cite": "Commission Implementing Regulation (EU) 2025/2620 of 16 December 2025 — "
                "calculation of the free allocation adjustment to the number of CBAM certificates "
                "(Art. 31 of Reg. 2023/956): CBAM benchmark × CBAM factor × cross-sectoral "
                "correction factor × quantity of goods.",
        "url": "https://eur-lex.europa.eu/eli/reg_impl/2025/2620/oj",
        "used_for": "무상할당 공제 = benchmark × CBAM factor — 핵심 계산식",
    },
    "EC_CBAM_QA": {
        "cat": "regulation",
        "date": "definitive period version (2026)",
        "cite": "European Commission, Carbon Border Adjustment Mechanism (CBAM) Questions and Answers — "
                "Q3.7 (certificates = embedded emissions − embedded free allocation − carbon price paid), "
                "Q3.8 (free allocation adjustment), Q3.10 (carbon price paid in third countries), "
...
TOOLTIPS = {
    ...
    "Phase-in factor": (
        "연도별 phase-in (= 100% − CBAM factor)\n"
        "■ 2026 2.5%, 2030 48.5%, 2034 100%\n"
        "■ benchmark분 무상할당 공제에만 적용 — benchmark 초과분은 전액 부과"
    ),
    "Embedded emissions": (
...
def calc_unit_cbam(SEE: float, benchmark: float, eua_price_eur: float,
    ...
    → benchmark 초과분은 2026년부터 전액 부과되고, phase-in으로 늘어나는 건
      benchmark분 공제가 줄어드는 몫이다. 2034년부터는 내재배출 전량에 부과.
...
already_zero = result["gross_unit_cost_eur"] == 0
...
if already_zero:
    ...
    insight_msg = (
        ...
        f"<span class='warn'>부분 CCS</span>로 초과분부터 줄이는 게 효과적."
    )
else:
    insight_class = "bad"
    insight_header = f"⚠ {company_name}"
...
**CCS 회피액** = CCS 도입 전후 CBAM 부담 차이 (포집률 η=90%)

📚 출처:
- {ref_link("EU_REG_2023_956", "EU Regulation 2023/956")} — CBAM 본법
- {ref_link("EU_IR_2025_2620", "EU IR 2025/2620")} — 무상할당 공제 계산
...
""",
    ...
    for opt in ABATEMENT_OPTIONS:
        ...
        achieves = new_SEE <= req["target"]
        rows.append({
            "수단": opt["label"],
            "감축율": f"-{opt['reduction_pct']:.0f}%",
            "잔존 SEE": f"{new_SEE:.3f}",
            f"CBAM 0 ({analysis_year})": "✅" if achieves else "❌",
    ...
    n_total_techs = len(ccus_data.get("technologies", {}))
    fit_techs = set(CCUS_SECTOR_FIT.get(sector["ccus_sector"], []))
    all_techs = list(ccus_data["technologies"].keys())

    # 데이터 모드 표시 (live vs fallback)
    mode_badge_color = "#81C784" if ccus_mode == "live" else "#FFB74D"
    mode_badge_label = "✅ LIVE (자매 도구)" if ccus_mode == "live" else "⚠️ FALLBACK (네트워크 실패)"
    st.markdown(
        f"##### 💰 4-C. CCS 도입 시 BEP 분석 — 전체 {n_total_techs}개 기술 비교 "
        f"<span style='background:{mode_badge_color}22;color:{mode_badge_color};"
        f"font-size:0.7rem;padding:2px 8px;border-radius:4px;font-weight:500;'>"
        f"{mode_badge_label}</span>",
        unsafe_allow_html=True,
    )
    ...
    for tk in all_techs:
        ...
        ccs_cost_usd = coca_usd_per_t * avoided["captured_co2_t"]
        avoided_usd = avoided["avoided_annual_eur"] / fx_eur_usd
        net_usd = avoided_usd - ccs_cost_usd
        bep_rows.append({
            "추천": "⭐" if is_recommended else "",
            "CCUS 기술": tdata.get("name", tk),
            "Short": tdata.get("short_name", ""),
            "Capture η": f"{cap_rate*100:.0f}%",
            "COCA (USD/t)": f"${coca_usd_per_t:.0f}",
    ...
    st.markdown(
        f"""
> 💡 **해석**: 자매 CCUS 도구의 **{n_total_techs}개 기술 모두** live fetch (마지막 갱신: `{last_updated}`).
> ⭐는 본 sector 적합 기술 ({n_recommended}개). 순익 내림차순 정렬 — 가장 위가 가장 경제적.
> COCA = CAPEX × 0.10 (CRF 근사) + OPEX. 자세한 기술 비교는 [자매 도구 ↗]({CCUS_APP_URL}) 참조.
> 단, 위 분석은 **단년도 정태 분석** — 가동개시·ramping·할인율은 아래 4-D 참조.
"""
    )
    ...
    npv_tech = st.selectbox(
        ...
        options=all_techs,
        index=0,
        format_func=lambda k: ccus_data["technologies"][k].get("name", k),
        key="npv_tech_select",
    )
    ...
    npv_result = ccs_npv_analysis(
        ...
        ccs_capex_usd_per_tpy=get_tech_capex(ccus_data, npv_tech),
        ccs_opex_usd_per_tco2=get_tech_opex(ccus_data, npv_tech),
    ...
    if npv_result["npv_net_eur"] > 0:
        st.success(
            f"✅ {npv_tdata.get('name', npv_tech)} 도입 권장 — "
            f"NPV 순익 €{npv_result['npv_net_eur']/1e6:+,.1f}M, "
            f"BEP {npv_result['bep_year'] or 'N/A'}년."
        )
    else:
        st.warning(
            f"⚠️ {npv_tdata.get('name', npv_tech)} 도입 시 NPV 적자 "
...
<h4 style='margin:0 0 8px 0;'>🌫️ 자매 도구: CCUS 기술 벤치마크 (Phase 2 활성화)</h4>
한국·미국·EU의 9개 CCUS 흡수제·공정 기술의 COCA·SPECCA·CAPEX 비교 도구.<br>
이 CBAM 계산기와 <strong>live fetch 연계</strong>되어 sector별 적합 기술 자동 추천 + BEP 분석 제공.<br>
자매 도구의 <code>data/ccus_metrics.json</code>이 single source of truth.<br><br>
...
<br><br>
<small>📡 데이터 모드: <code>{ccus_mode}</code> ·
마지막 갱신: <code>{ccus_data.get('last_updated', 'n/a')}</code> ·
24h 캐시 (자동 동기화)</small>
</div>
""",
        unsafe_allow_html=True,
    ...
    for tk in all_techs:
        ...
        if not tdata:
            continue
        is_recommended = tk in fit_techs
        coca = get_tech_coca(ccus_data, tk)
        capex = get_tech_capex(ccus_data, tk)
        opex = get_tech_opex(ccus_data, tk)
        cap_rate = get_tech_capture_rate(ccus_data, tk)
        rec_rows.append({
            "추천": "⭐" if is_recommended else "",
            "기술": tdata.get("name", tk),
            "Short": tdata.get("short_name", ""),
            "Category": tdata.get("category", "-"),
            "COCA (USD/t)": f"${coca:.0f}",
            "CAPEX (USD/tpy)": f"${capex:.0f}",
            "OPEX (USD/t)": f"${opex:.1f}",
            "Capture η": f"{cap_rate*100:.0f}%",
            "TRL": tdata.get("TRL", "-"),
            "_coca": coca,
        })
    ...
    st.markdown(
        f"""
##### ✅ Phase 2 활성화 완료 (2026-05)

| 항목 | 상태 |
|---|---|
| 자매 도구 repo의 `data/ccus_metrics.json` (single source of truth) | ✅ |
| `load_ccus_metrics()` GitHub raw URL fetch | ✅ live |
| 24h 캐시 (`@st.cache_data ttl=86400`) | ✅ |
| CBAM 계산기의 sector_fit 매핑 (CBAM-specific) | ✅ 본 도구 유지 |
| Network 실패 시 fallback dict | ✅ |
| 자매 도구의 LIT 변경 시 24h 내 자동 반영 | ✅ |

**데이터 모드**: `{ccus_mode}` · **마지막 갱신**: `{ccus_data.get('last_updated', 'n/a')}` ·
**source**: <a href='{CCUS_JSON_URL}' target='_blank'>raw JSON ↗</a>

##### 📋 Phase 3 로드맵 (향후)

- [ ] 양 도구의 cross-validation (POSCO·시멘트 retrofit cost)
- [ ] 한국 sector × CCUS 기술 매트릭스 히트맵
- [ ] Multi-sector 포트폴리오 분석
"""
    )
...
with tabs[10]:
    ...
    if usage_mode == "fallback":
        st.warning(
            ...
            "GitHub repo에서 직접 보기: "
            "[USAGE.md ↗](https://github.com/cafeon90-oss/CBAM_calculator/blob/main/USAGE.md)"
        )
    else:
        # 1.2 다이어그램 위치에서 SVG로 대체 — markdown은 ![](./xxx.svg) 라서 GitHub에선 보이지만
        # Streamlit에선 직접 SVG render 필요
        usage_md_for_streamlit = usage_md.replace(
...
문의·기여: [GitHub issue](https://github.com/cafeon90-oss/CBAM_calculator/issues) · [email](mailto:cafeon90@gmail.com)
"""
    )
